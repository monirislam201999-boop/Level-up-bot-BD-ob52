...
try:
    from important_zitado import create_protobuf_packet
except ImportError:
    print("Warning: important_zitado not found. Some functionality might fail.")
    def create_protobuf_packet(fields): return b'' # Placeholder
...
# ================== CONFIG FROM MAIN1 ==================
START_SPAM_DURATION = 1.5      # Keep short to prevent cancelling
WAIT_AFTER_MATCH_SECONDS = 15  # Reduced from 20 to 15 for faster restart
START_SPAM_DELAY = 0.5         # Keep delay for reliability
...
def start_autooo(key, iv):
    ...
    if len(header_length_final) == 2:
        final_packet = "0515000000" + header_length_final + nmnmmmmn(packet, key, iv)
    elif len(header_length_final) == 3:
        final_packet = "051500000" + header_length_final + nmnmmmmn(packet, key, iv)
    elif len(header_length_final) == 4:
        final_packet = "05150000" + header_length_final + nmnmmmmn(packet, key, iv)
    elif len(header_length_final) == 5:
        final_packet = "0515000" + header_length_final + nmnmmmmn(packet, key, iv)
...
async def auto_start_loop(team_code, uid, key, iv):
    global stop_auto, online_writer, whisper_writer, chat_id
    ...
    while not stop_auto:
        try:
            # join
            # Using GenJoinSquadsPacket from main2's imported xC4
            ...
            if chat_id:
                ...
                P = await SEndMsG(0, msg, uid, chat_id, key, iv)
                await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)

            # start spam
            start_packet = start_autooo(key, iv)
            ...
            waited = 0
            while waited < WAIT_AFTER_MATCH_SECONDS and not stop_auto:
            ...
            # leave
            leave_packet = leave_s(key, iv)
            if online_writer:
                online_writer.write(leave_packet)
                await online_writer.drain()
                await asyncio.sleep(0.5)
                online_writer.write(leave_packet) # Send leave packet again to ensure
                await online_writer.drain()
            print(f"[AUTO] Left team {team_code} to rejoin again.")
            await asyncio.sleep(2) # Reduced back to 2s for speed
            ...
            # Network issue, might need restart or break
            stop_auto = True
            break
...
async def TcPChaT(ip, port, AutHToKen, key, iv, LoGinDaTaUncRypTinG, ready_event, region , reconnect_delay=0.5):
    ...
    global spam_room , whisper_writer , spammer_uid , spam_chat_id , spam_uid , online_writer , chat_id , XX , uid , Spy,data2, Chat_Leave, stop_auto
    while True:
        try:
            reader , writer = await asyncio.open_connection(ip, int(port))
            ...
            while True:
                ...
                if data.hex().startswith("120000"):
                    ...
                    if response:
                        ...
                        # === Logicbot Integration: /auto command ===
                        if inPuTMsG.startswith('/auto '):
                            try:
                                ...
                                if len(parts) > 1:
                                    ...
                                    asyncio.create_task(auto_start_loop(team_code, uid, key, iv))
                                    msg = f"[C][B][00FF00]Auto Loop Started for Team: {team_code}"
                                    P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                                    await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                            except Exception as e:
                                print(f"Error starting auto loop: {e}")

                        if inPuTMsG.startswith('/stopauto'):
                            stop_auto = True
                            msg = f"[C][B][FF0000]Auto Loop Stopping..."
                            P = await SEndMsG(response.Data.chat_type, msg, uid, chat_id, key, iv)
                            await SEndPacKeT(whisper_writer, online_writer, 'ChaT', P)
                        # ============================================
...
async def MaiiiinE():
    Uid , Pw = '4355692430','A9F0A0997A35BD62BCAB2DF69D95A43C02EDF4FE1A5622C24F4D236A6CB508CC'
